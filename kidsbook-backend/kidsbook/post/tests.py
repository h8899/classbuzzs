...
class TestPost(APITestCase):
    ...
    def check_matching_fields(self, expected, current):
    ...
    def test_create_post_in_group(self):
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../backend/media/picture.png'), 'rb') as pic:
            response = self.client.post(url, {"content": "testing content", "link": "http://ogp.me",
                "picture": pic, 'is_sponsored' : 'true', 'is_announcement': 'true'}, HTTP_AUTHORIZATION=self.creator_token)
        ...
        expected_stats = {
            "content": "testing content", "link": "http://ogp.me",
            'is_sponsored' : True, 'is_announcement': True
        }
        self.assertTrue(self.check_matching_fields(expected_stats, post_data))

        # Create another post
        url = "{}/group/{}/posts/".format(url_prefix, self.group_id)
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../backend/media/picture.png'), 'rb') as pic:
            response = self.client.post(url, {"content": "testing content2", "link": "http://ogp.me",
                "picture": pic, 'is_sponsored': True}, HTTP_AUTHORIZATION=self.creator_token)
        self.assertTrue(202, response.status_code)

        post_data = response.data['data']
        expected_stats = {
            "content": "testing content2", "link": "http://ogp.me",
            'is_sponsored' : True, 'is_announcement': False
        }
        self.assertTrue(self.check_matching_fields(expected_stats, post_data))
    ...
    def test_update_post_detail_with_id(self):
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../backend/media/picture.png'), 'rb') as pic:
            request_changes = {"content": "Changed content", "link": "http://ogp.me", "picture": pic, 'is_random': True}
...
class TestCensorship(APITestCase):
    def setUp(self):
        ...
        # Create a post
        self.post = Post.objects.create_post(
            content='Stop talking SH1t, n1gg4',
            creator=self.creator,
            group=self.group
        )
    ...
    def test_create_a_post_with_profanity_with_confirm(self):
        url = "{}/group/{}/posts/".format(url_prefix, self.group_id)
        response = self.client.post(
            url,
            {"content": "You b1tCh", "link": "http://ogp.me", "confirm_profanity": True},
            HTTP_AUTHORIZATION=self.member_token
        )
        self.assertEqual(202, response.status_code)

    def test_create_a_post_with_profanity_with_confirm_wrong(self):
        url = "{}/group/{}/posts/".format(url_prefix, self.group_id)
        response = self.client.post(
            url,
            {"content": "You b1tCh", "link": "http://ogp.me", "confirm_profanity": 'true'},
    ...
    def test_create_a_post_with_profanity_with_confirm_wrong_2(self):
        url = "{}/group/{}/posts/".format(url_prefix, self.group_id)
        response = self.client.post(
            url,
            {"content": "You b1tCh", "link": "http://ogp.me", "confirm_profanity": 'false'},
            HTTP_AUTHORIZATION=self.member_token
        )
    ...
    def test_create_a_post_with_profanity_with_confirm_wrong_2(self):
        ...
        response = self.client.post(
            url,
            {"content": "You b1tCh", "link": "http://ogp.me", "confirm_profanity": 'abc'},
            HTTP_AUTHORIZATION=self.member_token
        )
        self.assertEqual(405, response.status_code)

    def test_get_censor_post_from_superuser(self):
    ...
    def test_get_censor_comment_from_member(self):
    ...
    def test_create_a_post_with_profanity(self):
        url = "{}/group/{}/posts/".format(url_prefix, self.group_id)
        response = self.client.post(
            url,
            {"content": "You b1tCh", "link": "http://ogp.me"},
            HTTP_AUTHORIZATION=self.member_token
        )
        self.assertEqual(405, response.status_code)


class TestPostGroupSetting(APITestCase):
    def setUp(self):
        # Creator
        self.username = "john"
        self.email = "john@snow.com"
    ...
    def test_update_group_setting(self):
        # Disable like post in GroupSetting
        payload = { "is_like_enabled": False, "is_share_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
            payload,
    ...
    def test_update_group_setting_by_non_superuser(self):
        # Disable like post in GroupSetting
        payload = { "is_like_enabled": False, "is_share_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
    ...
    def test_update_group_setting_by_non_member(self):
        # Disable like post in GroupSetting
        payload = { "is_like_enabled": False, "is_share_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
    ...
    def test_like_post_comment_when_disabled(self):
        # Disable like post in GroupSetting
        payload = { "is_like_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
            payload,
            HTTP_AUTHORIZATION=self.creator_token)
        ...
        self.assertEqual(
            ...
            'Liking is disabled for this group'
        )
    ...
    def test_like_post_comment_when_disabled_by_member(self):
        # Disable like post in GroupSetting
        payload = { "is_like_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
        ...
        # Post
        url = "{}/post/{}/likes/".format(url_prefix, self.post.id)
        response = self.client.post(url, {"like_or_dislike": True}, HTTP_AUTHORIZATION=self.member_token)
        self.assertEqual(405, response.status_code)
    ...
    def test_like_post_comment_when_disabled_by_non_member(self):
        # Disable like post in GroupSetting
        payload = { "is_like_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
    ...
    def test_create_comment_when_disabled(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_comment_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
            payload,
            HTTP_AUTHORIZATION=self.creator_token)
        ...
        url = "{}/post/{}/comments/".format(url_prefix, self.post.id)
        response = self.client.post(url, {"content": "unique_comment"}, HTTP_AUTHORIZATION=self.creator_token)
        self.assertEqual(405, response.status_code)

    def test_create_comment_when_disabled_by_non_superuser(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_comment_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
        ...
        url = "{}/post/{}/comments/".format(url_prefix, self.post.id)
        response = self.client.post(url, {"content": "unique_comment"}, HTTP_AUTHORIZATION=self.member_token)
        self.assertEqual(405, response.status_code)

    def test_create_comment_when_disabled_by_non_member(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_comment_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
        ...
        url = "{}/post/{}/comments/".format(url_prefix, self.post.id)
        response = self.client.post(url, {"content": "unique_comment"}, HTTP_AUTHORIZATION=self.user_token)
        self.assertEqual(403, response.status_code)

    def test_share_post_when_disabled(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_share_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
            payload,
            HTTP_AUTHORIZATION=self.creator_token)
    ...
    def test_share_post_when_disabled_by_non_superuser(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_share_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
    ...
    def test_share_post_when_disabled_by_non_member(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_share_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
    ...
    def test_flag_post_when_disabled(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_flag_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
            payload,
            HTTP_AUTHORIZATION=self.creator_token)
    ...
    def test_flag_post_when_disabled_by_non_superuser(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_flag_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
        ...
        url = "{}/post/{}/flags/".format(url_prefix, self.post.id)
        flag_status = "UNDER APPROVAL"
        response = self.client.post(url, {"status": flag_status}, HTTP_AUTHORIZATION=self.member_token)
        self.assertEqual(405, response.status_code)

    def test_flag_post_when_disabled_by_non_member(self):
        # Disable create comment on post in GroupSetting
        payload = { "is_flag_enabled": False}
        response = self.client.post(
            "{}/group/{}/setting/".format(url_prefix, self.group_id),
        ...
        url = "{}/post/{}/flags/".format(url_prefix, self.post.id)
        flag_status = "UNDER APPROVAL"
        response = self.client.post(url, {"status": flag_status}, HTTP_AUTHORIZATION=self.user_token)
        self.assertEqual(403, response.status_code)
