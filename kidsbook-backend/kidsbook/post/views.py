...
class GroupPostList(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            # If no param `confirm_profanity` and the content has profanity
            if  (contains_profanity(request.data.get('content', ''))
                    and str(request.data.get('confirm_profanity', 'false')).strip().lower() != 'true'):
                return Response({'error': "Unable to create the post due to profanity"}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
